...
import AfterRnn from "../Web/DeepLearning/AfterRnn";
import PdfUserRnn from "../Web/DeepLearning/PdfUserRnn";
import PageLifeImg2Rnn from "../Web/DeepLearning/PageLifeImg2Rnn";
import ErrorValues from "../Web/DeepLearning/ErrorValues";

const { Option } = Select;
const activationOptionsRNN = [
  {
    label: "tanh",
    value: "tanh",
    description:
      "Hyperbolic Tangent (tanh): Değerleri [-1, 1] aralığına dönüştürür. RNN'lerde sıkça kullanılır.",
  },
  {
    label: "sigmoid",
    value: "sigmoid",
    description:
      "Sigmoid: Değerleri [0, 1] aralığına dönüştüren bir aktivasyon fonksiyonudur. RNN'lerde giriş kontrol mekanizmalarında sıkça kullanılır.",
  },
  {
    label: "relu",
    value: "relu",
    ...
    description:
      "ReLU (Rectified Linear Unit): Negatif girişler için sıfır, pozitif girişler için girişi döndürür.",
  },
  {
    label: "softmax",
    value: "softmax",
    description:
      "Softmax: RNN'lerde çoklu sınıflandırma problemleri için çıkış katmanında kullanılan bir aktivasyon fonksiyonudur.",
  },
  {
    label: "linear",
    value: "linear",
    description:
      "Linear: Doğrusal aktivasyon fonksiyonu, çıktıyı doğrudan geçirir.",
  },
];
...
const RNNForm = () => {
  ...
  const handleButtonClick = () => {
    ...
    form.validateFields().then((values) => {
      if (
        ...
        axios
          .post("http://localhost:5000/Lstm_pagelife2_img_rnn", formData, {
            responseType: "json",
          })
          .then((response) => {
            ...
            axios
              .get("http://localhost:5000/Lstm_pagelife2_img_rnn_path", {
                responseType: "blob",
              })
              .then((response) => {
                ...
                message.success("pagelife çizildi!");
                setCounter(1)
                return axios.get(
                  "http://localhost:5000/Lstm_pagelife_img_rnn_gpt_yorum"
                );
              })
              .then((response) => {
                ...
                message.success(
                  "page life verisi alındı!"
                );
                message.success("işlem tamamlandı")
                setCounter(2)
  ...
  const handleDefaultButtonClick = () => {
    const { noron, dropout, epoch, batch, optimizer, lossfunc, activation } =
      form.getFieldsValue([
        "noron",
        "dropout",
        "epoch",
  ...
  return (
    <div>
      <Form form={form} labelCol={{ span: 3 }}>
          ...
          label={
            <span>
              <Tooltip
                title={
                  <div style={{ textAlign: "justify" }}>
                    Aktivasyon fonksiyonları, sinir ağlarının her katmanında
                    non-linearite ekleyerek ve modelin öğrenme kapasitesini
                    artırarak sinir ağlarının daha karmaşık ilişkileri
        ...
        >
          <Select>
            {activationOptionsRNN.map((option,index) => (
      ...
      {finnish && <AfterRnn />}
      {finnish && <PageLifeImg2Rnn imageSrc={imageSrc} answer={answer} />}
